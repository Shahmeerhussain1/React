...
const complex = [
  { company: "XYZ", jobs: ["Javascript", "Eeact"] },
  { company: "ABC", jobs: ["AngularJs", "onic"] },
];
const posts = [
  {
    userId: 1,
  ...
  return (
    <div id="main">
      <div>
        ...
        <h3>When devloper have to export multiple components from file then no need to use brackets at the time of import but when 
          only one component Or Function have to export then the brackets are necessary at the time of import </h3>
      </div>
      <div id="one">{name}</div>
      <div id="two">{obj.name}</div>
      <div id="three">{data.map((z) => z)}</div>
      <div id="four">{list.map((p) => p.name)}</div>
      <div id="five">
        <div>
          {complex[0].company} {complex[0].jobs[0]} {complex[0].jobs[1]}
        </div>
        <div>{complex.map((v) => v.company)}</div>
      </div>
      <div id="fromLinkOne">
        {users.map((val) => {
          return (
